...
public class AdminTab {
    ...
    public static void build(ConfigBuilder builder, ZXCHelperConfig config, Identifier bg) {
        if(!ZXCHelper.getInstance().getPlayerRole().hasPermission(Role.OWNER)) return;
        ConfigEntryBuilder entryBuilder = builder.entryBuilder();
        ConfigCategory category = builder.getOrCreateCategory(Text.literal("Test"));
        category.setBackground(bg);
        category.setCategoryBackground(bg);
        category.addEntry(entryBuilder.startStrList(Text.literal("Test"), config.getSoundWheelSounds())
